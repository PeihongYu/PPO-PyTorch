...
class FeatureFusion(nn.Module):
    def __init__(self, state_dim, out_dim):
        ...
        self.cnn1 = nn.Conv2d(1, 8, 8)
        self.relu1 = nn.ReLU()
        self.pool1 = nn.MaxPool2d(2, stride=1, padding=1)

        self.cnn2 = nn.Conv2d(8, 16, 4)
        self.relu2 = nn.ReLU()
        ...
        self.joint_nn  = nn.Linear(4608+16, out_dim)
    ...
    def forward(self, x):
        ...
        x1 = self.relu1(self.cnn1(x1))
        x1 = self.pool1(x1)
        x1 = self.relu2(self.cnn2(x1))
        x1 = self.pool2(x1)
        x1 = self.relu3(self.cnn3(x1))
        x1 = self.pool3(x1)
        x1 = self.flatten(x1)
